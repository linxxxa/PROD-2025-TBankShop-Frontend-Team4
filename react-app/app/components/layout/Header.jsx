...
const Header = ({ showBackButton, onBackClick }) => {
  ...
  const handleProfileClick = () => {
    if (user) {
      ...
      window.history.pushState({}, '', `/users/phone/${user.phone}`);
    } else {
      // Если пользователь не авторизован, открываем регистрацию
      window.history.pushState({}, '', '/users/create');
    }
    setIsModalOpen(true);
  };
  ...
  const handleLogin = async (formData) => {
    try {
      ...
      // Если локально не найден, проверяем через API
      try {
        const userProfile = await usersAPI.getProfile(formData.phone);
        if (userProfile) {
          setUser(userProfile);
          setIsModalOpen(false);
          window.history.pushState({}, '', '/');
        ...
        console.error('Ошибка API при входе:', apiError);
      }
    } catch (error) {
  ...
  const handleRegister = (formData) => {
    // Данные уже сохранены в Modal компоненте
    setUser(formData);
    setIsModalOpen(false);
    window.history.pushState({}, '', '/');
    // Уведомление показывается в Modal компоненте
  };
  ...
  return (
    <header className='header'>
      <div className="header__container">
        ...
        {/* Всегда показываем кнопку "Личный кабинет" */}
        <button 
